...
<html>
    ...
    <div class="row">
      ...
      <?php
        $consulta="SELECT * FROM noticias WHERE categoria='1' ORDER BY puntos DESC;";
        $res=$mysqli->query($consulta);
        while ($dato=$res->fetch_assoc()) {
          print ('<div class="card border-primary">');
          print('<div class="card-header bg-transparent border-primary">'.$dato['autor'].'</div>');
          ...
          print('<button class="btn btn-outline-primary" type="submit" name="id" value="'.$dato['id'].'">Ir a la noticia</button>');
          print('</form>');
          print('</div>');
          print('<div class="card-footer bg-transparent border-primary">'.$dato['fecha'].'</div>');
          print('</div>');
          print('<br><br><br>');
        }
       ?>
