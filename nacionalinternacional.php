...
<html>
    <head>
        <title>Nacional e Internacional</title>
    		<meta charset="utf-8">
    ...
    <div class="row">
      <?php
        $consulta="SELECT * FROM noticias WHERE categoria='2' ORDER BY puntos DESC;";
        $res=$mysqli->query($consulta);
        while ($dato=$res->fetch_assoc()) {
          print ('<div class="card border-warning">');
          print('<div class="card-header bg-transparent border-warning">'.$dato['autor'].'</div>');
          ...
          print('<button class="btn btn-outline-warning" type="submit" name="id" value="'.$dato['id'].'">Ir a la noticia</button>');
          print('</form>');
          print('</div>');
          print('<div class="card-footer bg-transparent border-warning">'.$dato['fecha'].'</div>');
          print('</div>');
          print('<br><br><br>');
        }
       ?>
